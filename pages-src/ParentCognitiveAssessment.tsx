...
export default function ParentCognitiveAssessment() {
  ...
  const loadAssessmentSession = useCallback(async () => {
    ...
    try {
      ...
      if (existingQuestions && !questionsError) {
        ...
        const { generateCognitiveAssessment } = await import('@/services/gemini-cognitive-generator');
  ...
  }, [loadAssessmentSession, assessmentType]); // Reload when assessment type changes
  ...
  const handleStartVoiceSession = async () => {
    ...
    try {
      ...
      // Generate LiveKit access token with RoomConfiguration for automatic agent dispatch
      // This token includes RoomConfiguration which automatically dispatches the agent
      // when the participant joins - no separate dispatch call needed
      toast.info('Generating secure connection token with agent dispatch...');
      ...
      // Generate token with RoomConfiguration - this automatically dispatches the agent
      ...
      // The agent-jwt endpoint now returns a LiveKit access token (not a custom JWT)
      // This token includes RoomConfiguration for automatic agent dispatch
      const token = agentTokenResponse.token;
      const wsUrl = agentTokenResponse.wsUrl || liveKitUrl;
      
      // Connect to LiveKit room using the token with RoomConfiguration
      // The agent will be automatically dispatched when we join
      toast.info('Connecting to room (agent will join automatically)...');
