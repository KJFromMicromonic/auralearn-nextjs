...
- `PROBLEM_AND_PRODUCT.md` - Problem understanding, persona analysis, and product excellence documentation
- `TECH_STACK.md` - Complete tech stack documentation and architecture overview
- `MIGRATION_NOTES.md` - Detailed migration notes
- `CONVERT_PAGES.md` - Guide for converting pages
- `VERCEL_BUILD_FIX.md` - Vercel deployment configuration
...
### Adaptability Challenge Sessions
- Run `supabase-challenge-sessions-schema.sql` on your Supabase instance to ensure the `challenge_sessions` table (and the new `problem_statement` column) exist.
- The `/api/challenges/session` route now asks MistralAI to generate subject-specific problems plus materials, steps, coaching, and success criteria.
- Parents can leave and return to `/parent/challenges` without losing their active session—the UI reloads the saved session and shows the exact problem again.
- `/api/challenges/progress` exposes challenge history and streaks via the service role key so parents always see their active challenge, metrics, and streaks after refreshing.
- Challenge sessions respect each parent's language preference (managed by `LanguageContext`). Set the UI to French to receive the AI-generated prompts, steps, and reflections entirely in French.
