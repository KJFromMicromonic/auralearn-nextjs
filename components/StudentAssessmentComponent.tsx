...
import { generateAssessmentQuestions } from "@/services/assessment-api-client";
...
interface StudentAssessmentComponentProps {
  classId?: string;
  studentId?: string;
  token?: string;
}

export default function StudentAssessmentComponent({ classId, studentId, token }: StudentAssessmentComponentProps) {
  ...
  const loadStudentByToken = useCallback(async () => {
    setIsLoading(true);
  ...
  const loadStudentAndQuestions = useCallback(async () => {
    setIsLoading(true);
  ...
  }, [classId, studentId, toast, t]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [token, classId, studentId, loadStudentByToken, loadStudentAndQuestions]);

  interface StudentDataForAssessment {
    id: string;
    name: string;
    class_id: string;
  ...
  const loadQuestionsForStudent = async (studentData: StudentDataForAssessment) => {
    try {
  ...
  const submitAssessment = async (finalAnswers: Answer[]) => {
    ...
    try {
      ...
      if (errorMessage?.includes('permission denied') || errorMessage?.includes('RLS') || errorCode === '42501') {
        ...
        errorMessage = "Invalid student ID. Please contact your teacher.";
        ...
        <Card className="w-full max-w-2xl">
          ...
          <CardContent className="space-y-6">
            ...
            {accessMethod === 'manual_selection' && (
              <Button
                onClick={() => router.push(`/student-selection/${classId}`)}
                className="w-full"
                size="lg"
              >
                {t('common.back')}
