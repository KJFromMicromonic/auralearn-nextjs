...
import { generateAssessmentSummary } from "@/services/assessment-ai-summary-client";
...
export default function DashboardPage() {
  ...
  const loadStudentAssessmentData = useCallback(async () => {
    setIsLoading(true);
    try {
  ...
  }, [user?.id, loadStudentAssessmentData]);
  ...
  const generateStudentReport = async (studentId: string, studentName: string) => {
    try {
      ...
      console.log('Generating report for assessment ID:', assessmentId);

      // Generate report data
      const reportData = await prepareReportData(assessmentId);
      if (!reportData) {
        console.error('prepareReportData returned null for assessment ID:', assessmentId);
        console.error('Assessment data:', assessments[0]);
        throw new Error('Failed to prepare report data. The assessment may be missing student or class information.');
      }
      
      console.log('Report data prepared successfully:', {
        studentName: reportData.studentName,
        className: reportData.className,
        score: reportData.assessment.score,
      });
      ...
      console.log('About to generate PDF report...');
          ...
          {studentsWithLevels.length === 0 ? (
            ...
            <>
              <Card className="p-6 rounded-2xl">
                ...
                <div className="grid grid-cols-6 gap-3">
                  {studentsWithLevels.map((student) => (
                    <div
                      key={student.id}
                      onClick={() => router.push(`/student-guide/${student.id}`)}
              ...
              <div className="flex justify-end gap-4">
                ...
                <Button onClick={() => router.push("/insights")} size="lg" className="rounded-xl px-8">
